...
1. Once you pushed all the changes to Gihub and merged them to master, create a new release and add a version tag to it - i.e. `v1.7.0`;

2. Open your terminal application of choice and type the following commands:

    ```
    docker run -it python:3.12.5-bookworm /bin/bash
    pip install 'git+https://github.com/numbworks/nwshared.git@v1.7.0#egg=nwshared&subdirectory=src'
